...
/***/ function(module, exports, __webpack_require__) {
	...
	__webpack_require__(/*! ./src/polyfill.js */8);
	module.exports = __webpack_require__(/*! ./src/core.js */9);
...
/***/ function(module, exports, __webpack_require__) {
	...
	_jquery2.default.fn.extend({
	    ...
	    ffInit: function ffInit(extentions) {
	        console.log("ffInit");
	        var $this = this;
	        var p = parse();
	        var node = _utility2.default.getExtention(extentions, p.name);
	        var result;
	        if (!node) return;
	    ...
	    ffVmSetup: function ffVmSetup(vm, tokens) {
	    ...
	    ffAddDataBind: function ffAddDataBind(tokens) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _ = {
	    ...
	    unwrapValueAccessor: function unwrapValueAccessor(valueAccessor) {
	        //todo: find places that should use this function 
	    ...
	    parseValue: function parseValue(str, vm) {
	    ...
	    addToSortOrder: function addToSortOrder(list, value) {
	        var result = [];
	    ...
	    getCommand: function getCommand(value) {
	        var inverse = false;
	        var plus = false;
	        var key = value;
	        if (value[0] === "-") {
	    ...
	    unwind: function unwind(obj, str) {
	        var ary = str && str.split(".");
	        var o = _knockout2.default.utils.peekObservable(obj);
	    ...
	    safeVmNode: function safeVmNode(vm, target, defaultValue) {
	        var result = vm[target];
	        if (!result) {
	            vm[target] = _knockout2.default.observable(defaultValue);
	            result = vm[target];
	        }
	        return result;
	    },
	    //todo: document this helper
	    getExtention: function getExtention(extend, name) {
	    ...
	    addDataBind: function addDataBind($elm, bind, target) {
	        var oldBind;
	    ...
	    objectToArray: function objectToArray(targetObj) {
	        var result = [],
	            key,
	            obj,
	            unwrapped;
	    ...
	    filterObject: function filterObject(targetObj, callback) {
	        var result = {},
	            key,
	            obj,
	            unwrapped;
	    ...
	    setupType: function setupType($elm, observable) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	_knockout2.default.bindingHandlers.ffEvent = {
	...
	_knockout2.default.bindingHandlers.ffInlineUpdate = {
	    init: function init(element, valueAccessor, allBindings, viewModel, bindingContext) {
	        var ESC = 27;
	        var LF = 10;
	...
	_knockout2.default.bindingHandlers.ffChangeEvent = {
	    init: function init(element, valueAccessor, allBindings, viewModel, bindingContext) {
	        if (valueAccessor && valueAccessor()) {
	...
	_knockout2.default.bindingHandlers.ffSubmit = {
	    init: function init(element, valueAccessor, allBindings, viewModel, bindingContext) {
	        _knockout2.default.utils.registerEventHandler(element, "submit", function (event) {
	            if (event.preventDefault) {
	...
	_knockout2.default.bindingHandlers.ffImg = {
	    init: function init(element, valueAccessor, allBindings, viewModel, bindingContext) {
	        var value = _knockout2.default.utils.unwrapObservable(valueAccessor());
	        if (value) {
	...
	_knockout2.default.bindingHandlers.ffVisible = {
	    //todo: rename get Objects Length, move into update
	    testForObject: function testForObject(valueAccessor) {
	        var list, length;
	        //If the valueAccessor is not an object, return itself.  
	        //If the valueAccessor is an object, retrun count of attributes
	        var unwrapped = _utility2.default.unwrapValueAccessor(valueAccessor);
	        if (_utility2.default.isObject(unwrapped)) {
	            list = _utility2.default.objectToArray(unwrapped);
	            length = list.reduce(countProperties, 0);
	            return _utility2.default.wrap(length);
	        }
	        return valueAccessor;
	    ...
	    // },
	    update: function update(element, valueAccessor, allBindings, viewModel, bindingContext) {
	        var ffValueAccessor = _knockout2.default.bindingHandlers.ffVisible.testForObject(valueAccessor);
	        return _knockout2.default.bindingHandlers.visible.update(element, ffValueAccessor, allBindings, viewModel, bindingContext);
	    }
	};
	
	_knockout2.default.bindingHandlers.ffWith = {
	    init: function init(element, valueAccessor, allBindings, viewModel, bindingContext) {
	        function ffValueAccessorComputed() {
	            var x = viewModel._hash().substr(1) || 'bar';
	            return viewModel[x](); //unwrap
	        }
	        var ffValueAccessor = _knockout2.default.computed(ffValueAccessorComputed);
	        return _knockout2.default.bindingHandlers.with.init(element, ffValueAccessor, allBindings, viewModel, bindingContext);
	    }
	};
	
	_knockout2.default.bindingHandlers.ffForEachObject = {
	    makeTemplateValueAccessor: function makeTemplateValueAccessor(valueAccessor, element, bindingContext) {
	        return function () {
	            var $elm = $(element),
	                modelValue = valueAccessor(),
	                unwrappedValue = _knockout2.default.utils.peekObservable(modelValue),
	                // Unwrap without setting a dependency here
	            sortOrder = $elm.attr("atr-sortorder"),
	                filter = $elm.attr("atr-filter"),
	                rewrapped;
	            _knockout2.default.utils.extend(bindingContext, {
	                $list: _utility2.default.unwrapValueAccessor(valueAccessor),
	            ...
	            sortOrder && (unwrappedValue = unwrappedValue.sort(_utility2.default.sort(sortOrder)));
	            function filterCallback(object) {
	                var filterParts = filter.split('=');
	                var key = filterParts[0];
	                var unwrappedObject = _knockout2.default.utils.peekObservable(object);
	                var firstValue = unwrappedObject[key] && _knockout2.default.utils.peekObservable(unwrappedObject[key]);
	                var secondValue = filterParts.length && filterParts[1];
	                return firstValue === secondValue;
	            }
	            rewrapped = _knockout2.default.observableArray(unwrappedValue);
	
	            // If unwrappedValue is the array, pass in the wrapped value on its own
	            // The value will be unwrapped and tracked within the template binding
	            // (See https://github.com/SteveSanderson/knockout/issues/523)
	            if (!unwrappedValue || typeof unwrappedValue.length == "number") return { 'foreach': rewrapped, 'templateEngine': _knockout2.default.nativeTemplateEngine.instance };
	
	            // If unwrappedValue.data is the array, preserve all relevant options and unwrap again value so we get updates
	            _knockout2.default.utils.unwrapObservable(rewrapped);
	            return {
	                'foreach': unwrappedValue['data'],
	                'as': unwrappedValue['as'],
	                'includeDestroyed': unwrappedValue['includeDestroyed'],
	                ...
	                'beforeMove': unwrappedValue['beforeMove'],
	                'afterMove': unwrappedValue['afterMove'],
	                'templateEngine': _knockout2.default.nativeTemplateEngine.instance
	            };
	        };
	    },
	    init: function init(element, valueAccessor, allBindings, viewModel, bindingContext) {
	        return _knockout2.default.bindingHandlers['template']['init'](element, _knockout2.default.bindingHandlers['ffForEachObject'].makeTemplateValueAccessor(valueAccessor, element, bindingContext), allBindings, viewModel, bindingContext);
	    },
	    update: function update(element, valueAccessor, allBindings, viewModel, bindingContext) {
	        //Odd: if I don't use this next line, this function does not get called on update
	        _knockout2.default.utils.unwrapObservable(valueAccessor());
	        return _knockout2.default.bindingHandlers['template']['update'](element, _knockout2.default.bindingHandlers['ffForEachObject'].makeTemplateValueAccessor(valueAccessor, element, bindingContext), allBindings, viewModel, bindingContext);
	    }
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _firebase = __webpack_require__(/*! firebase */ 10);
	
	var _firebase2 = _interopRequireDefault(_firebase);
	
	var _knockoutMapping = __webpack_require__(/*! knockout-mapping */ 11);
	
	var _knockoutMapping2 = _interopRequireDefault(_knockoutMapping);
	
	var _knockoutPunches = __webpack_require__(/*! knockout-punches */ 12);
	...
	var _docHelper = __webpack_require__(/*! ./doc-helper.js */ 13);
	
	var _docHelper2 = _interopRequireDefault(_docHelper);
	
	var _koExtenders = __webpack_require__(/*! ./ko-extenders.js */ 14);
	...
	$("script[data-doc-help]").each(function () {
	    (0, _docHelper2.default)();
	});
	...
	(function () {
	    var extend = (0, _extentions2.default)(ff);
	    if (!setRoot() && _knockout2.default.punches) return;
	    _knockout2.default.punches.interpolationMarkup.enable();
	        ...
	        for (property in json) {
	            if (json.hasOwnProperty(property)) {
	                if (vm.hasOwnProperty(property)) {
	                    ...
	                    vm[property] = _knockout2.default.observable(getValue()).extend({ path: path, save: property });
	                }
	            }
	        }
	        return vm;
	    ...
	    function applyBindingsOnce() {
	        ...
	        _knockout2.default.applyBindings(ff.vm);
	
	        ff.$root.removeClass("loading").addClass("ready").find("div.loading").remove();
	        applyBindingsOnce = function applyBindingsOnce() {};
	    }
	
	    function setupHash() {
	        ff.vm._hash = _knockout2.default.observable(location.hash);
	    ...
	    function proccessComputedObservables() {
	        ...
	        for (key in list) {
	            if (list.hasOwnProperty(key)) {
	                ff.vm["$" + key + "$"] = _knockout2.default.computed(list[key], ff.vm);
	            }
	        }
	    }
...
/***/ function(module, exports, __webpack_require__) {
	...
	function setupDocumentationPage() {
	    console.log("in setupDocumentationPage");
	    $.fn.buildTree();
...
/***/ function(module, exports, __webpack_require__) {
	...
	function setupKoExtenders(firebase) {
	    //ko extentions
	    _knockout2.default.extenders.save = function (target, options) {
	        target.subscribe(function (newVal) {
	            ...
	            data = _knockout2.default.mapping.toJS(newVal);
	            update[options] = data;
	            child.update(update);
	        });
	        return target;
	...
	function getExtentions(ff) {
	    var extentions = {
	        ...
	        'value': {
	            binding: 'value',
	            extend: '_default',
	            vmSetup: _utility2.default.empty
	        },
	        'with': {
	        ...
	        'visible': {
	            ...
	            addDataBind: function addDataBind($elm, tokens) {
	                _utility2.default.addDataBind($elm, this.binding, tokens.content);
	            },
	            binding: 'ffVisible',
	            vmSetup: _utility2.default.empty,
	            extend: '_default'
	        ...
	        'class': {
	            ...
	            addDataBind: function addDataBind($elm, tokens) {
	                ...
	                _utility2.default.addDataBind($elm, this.binding, target);
	        ...
	        'foreach': {
	            ...
	            vmSetup: function vmSetup($elm, vm, child) {
	                ...
	                if (!id) return;
	                if (!selected) {
	                    selected = _knockout2.default.observableArray([]);
	        ...
	        'sort': { //todo: move to _event
	            ...
	            extend: '_default',
	            binding: 'ffEvent',
	            click: function click(valueAccessor, allBindings, viewModel, bindingContext) {
	                var $elm = $(this);
	                ...
	                sortParts = _utility2.default.addToSortOrder(sortParts, $elm.data("_ffSortCommand"));
	        ...
	        'set': {
	            init: function init(elm, content) {
	                ...
	                if (_utility2.default.isIn(lastChar, ['+', '-', '*', '/'])) {
	                ...
	                $elm.data('_ffValue', _utility2.default.parseValue(array[1]));
	            ...
	            vmSetup: function vmSetup($elm, vm) {
	                ...
	                if (ff.vm[target]) {
	                    //if observable exists, extend so value is saved on changed, this may result in duplicate extend functions and don't know if this is a problem
	                    ff.vm[target].extend({ save: target });
	                } else {
	                    //else create observable and extend
	                    ff.vm[target] = _knockout2.default.observable().extend({ save: target }); //todo: if a later input elm has a default value, consider setting that value in a similar if clause for that input value
	                }
	            },
	            ...
	            addDataBind: function addDataBind($elm) {
	                ...
	                if (result) {
	                    ...
	                    _utility2.default.addDataBind($elm, "attr", "{'_ffkey':" + result + "}");
	                }
	                _utility2.default.addDataBind($elm, this.binding, "$root." + $elm.data('_ffBindTarget'));
	            },
	            id: function id($elm) {
	        ...
	        'count': {
	            ...
	            vmSetup: function vmSetup($elm, vm, child, tokens) {
	                ...
	                vm[bindTarget] = _knockout2.default.computed(function () {
	                    var unwrappedValue = _knockout2.default.utils.unwrapObservable(vm[listName]);
	                    unwrappedValue = _utility2.default.objectToArray(unwrappedValue);
	                    if (unwrappedValue && unwrappedValue.length) {
	                        return unwrappedValue.reduce(function (total, obj) {
	                            ...
	                            var shouldCount = unwrappedObject[condition] && _utility2.default.unwrap(unwrappedObject[condition]);
	                            if (result.not) {
	            ...
	            oldVmSetupForTotal: function oldVmSetupForTotal($elm, vm) {
	                var tb = $elm.data("_ffBindTarget");
	                var listName = $elm.data("_ffListTarget");
	                var condition = $elm.data("_ffCondition");
	                if (!tb) return;
	                vm[tb] = _knockout2.default.computed(function () {
	                    var unwrappedValue = _knockout2.default.utils.unwrapObservable(vm[listName]);
	                    unwrappedValue = _utility2.default.objectToArray(unwrappedValue);
	                    if (unwrappedValue && unwrappedValue.length) {
	                        return unwrappedValue.reduce(function (total, obj) {
	        ...
	        'login': {
	            init: function init(elm, content) {
	                ...
	                if (!_utility2.default.isIn(content, ['facebook'])) {
	                    return;
	                }
	            ...
	            vmSetup: function vmSetup($elm, vm) {
	                var targetBind = $elm.data('_ffBindTarget');
	                vm._user = _knockout2.default.observable({
	                    firstName: _knockout2.default.observable(""),
	                    lastName: _knockout2.default.observable(""),
	                    name: _knockout2.default.observable(""),
	                    imgUrl: _knockout2.default.observable(""),
	                    timezone: _knockout2.default.observable(""),
	                    ageMin: _knockout2.default.observable(""),
	                    gender: _knockout2.default.observable(""),
	                    id: _knockout2.default.observable(""),
	                    link: _knockout2.default.observable(""),
	                    locale: _knockout2.default.observable(""),
	                    isImgSilhouette: _knockout2.default.observable(false)
	            ...
	            click: function click(valueAccessor, allBindings, viewModel, bindingContext) {
	                ff.fb.authWithOAuthPopup("facebook", function (error, authData) {
	                    if (error) {
	                        ...
	                        ff.vm._user().firstName(_utility2.default.unwind(authData, "facebook.cachedUserProfile.first_name"));
	                        ff.vm._user().lastName(_utility2.default.unwind(authData, "facebook.cachedUserProfile.last_name"));
	                        ff.vm._user().name(_utility2.default.unwind(authData, "facebook.cachedUserProfile.name"));
	                        ff.vm._user().imgUrl(_utility2.default.unwind(authData, "facebook.cachedUserProfile.picture.data.url"));
	                        ff.vm._user().isImgSilhouette(_utility2.default.unwind(authData, "facebook.cachedUserProfile.picture.data.is_silhouette"));
	                        ff.vm._user().timezone(_utility2.default.unwind(authData, "facebook.cachedUserProfile.timezone"));
	                        ff.vm._user().ageMin(_utility2.default.unwind(authData, "facebook.cachedUserProfile.age_range.min"));
	                        ff.vm._user().gender(_utility2.default.unwind(authData, "facebook.cachedUserProfile.gender"));
	                        ff.vm._user().id(_utility2.default.unwind(authData, "facebook.cachedUserProfile.id"));
	                        ff.vm._user().link(_utility2.default.unwind(authData, "facebook.cachedUserProfile.link"));
	                        ff.vm._user().locale(_utility2.default.unwind(authData, "facebook.cachedUserProfile.locale"));
	                    }
	                });
	            },
	            id: function id() {
	        ...
	        'push': {
	            init: function init(elm, content) {
	                ...
	                target = array[0];
	                $elm.data('_ffValue', _utility2.default.parseValue(array[1], ff.vm)); //todo: don't pass in vm, this will not work for nested properties
	                $elm.data('_ffTarget', target);
	            ...
	            vmSetup: function vmSetup($elm, vm) {
	                ...
	                if (ff.vm[target]) {
	                    ...
	                    //else create observable and extend
	                    ff.vm[target] = _knockout2.default.observableArray().extend({ save: target }); //todo: if a later input elm has a default value, consider setting that value in a similar if clause for that input value
	                }
	            },
	            id: function id($elm) {
	                return $elm.data("_ffBindTarget");
	            ...
	            click: function click(valueAccessor, allBindings, viewModel, bindingContext) {
	                ...
	                child.set(_knockout2.default.unwrap(value));
	            }
	        },
	        'delete': { //Bug: removing last element doens't update vm.  
	            ...
	            click: function click(valueAccessor, allBindings, viewModel, bindingContext) {
	                ...
	                if (tokens && tokens.listTarget) {
	                    var testItem = function testItem(elm) {
	                        listItem = _utility2.default.unwind(elm, tokens.parameterTarget);
	                        if (_utility2.default.isUndefined(listItem)) {
	                    ...
	                    newList = _utility2.default.filterObject(oldList(), testItem);
	                    oldList(newList);
	                    return;
	                }
	                ...
	                if (_utility2.default.objectToArray(listVm()).length === 1) {
	                    listVm({});
	                }
	            ...
	            addDataBind: function addDataBind($elm, tokens) {
	                if (tokens && tokens.listTarget) {
	                    _utility2.default.addDataBind($elm, this.binding, "deleteFromList");
	                    return;
	                }
	        ...
	        'select': {
	            init: function init(elm, content, tokens) {
	                var resultTokens = _utility2.default.extend({}, tokens);
	                return resultTokens;
	            },
	            extend: '_default',
	            binding: 'ffEvent',
	            ...
	            addDataBind: function addDataBind($elm) {
	                _utility2.default.addDataBind($elm, this.binding, "$root._select");
	            }
	        },
	        'toggle': {
	            init: function init(elm, content, tokens) {
	        ...
	        'populate': {
	            ...
	            click: function click(valueAccessor, allBindings, viewModel, bindingContext) {
	                ...
	                var temp = _knockout2.default.mapping.toJS(bindingContext.$data);
	            ...
	            addDataBind: function addDataBind($elm) {
	                _utility2.default.addDataBind($elm, this.binding, "$parent.populateForm");
	            }
	        },
	        ...
	        'submit': { //consider renaming submit to form or target or formTarget
	            ...
	            vmSetup: function vmSetup($elm, vm) {
	                ...
	                vm[formContent] = _knockout2.default.observable({});
	                //todo: dry
	                //todo: get path???
	                if (ff.vm[submitTarget]) {
	                    //if observable exists, extend so value is saved on changed, this may result in duplicate extend functions and don't know if this is a problem
	                    ff.vm[submitTarget].extend({ save: submitTarget });
	                } else {
	                    //else create observable and extend
	                    ff.vm[submitTarget] = _knockout2.default.observableArray([]).extend({ save: submitTarget }); //todo: if a later input elm has a default value, consider setting that value in a similar if clause for that input value
	            ...
	            click: function click(valueAccessor, allBindings, viewModel, bindingContext) {
	                ...
	                var data = _knockout2.default.mapping.toJS(bindingContext.$root[formContent]);
	                //var child = fbTarget.push();
	                var id = data._key;
	                if (id) {
	                    delete data._key;
	            ...
	            addDataBind: function addDataBind($elm) {
	                _utility2.default.addDataBind($elm, this.binding, "$root." + $elm.data('_ffBindTarget'));
	                _utility2.default.addDataBind($elm, "with", "$root." + $elm.data('_ffFormContent'));
	            }
...
/***/ }
/******/ ]);
//# sourceMappingURL=firefight-bundle.js.map